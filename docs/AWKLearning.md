...
It's named after it's investors.  
AlAho, Peter weinberger, Brian Kernigham.  

When it is developed ?
======================

It is developed at Bell Labs in the late 1970s.  

Why awk ?
=========

It was designed to perform simple data filtering and manupulation task.  

What is AWK good for ?
======================

AWK is great for manupulating text files which are divided in to records or lines, and in which each line is divided into Fields.  

AWK not so good for ?
=====================

AWK is not so good for manupulating binary data such a word and excel files.  
It is not well suited for parsing HTML.  

What versions of AWK exists ?
=============================

Original AWK : UNIX v7, 1978.  
New AWK (nAWK) : UNIX System, 1985.  
GNU AWK (gAWK) : Linux, 1990.  

### First AWK Program

```awk
$ awk '{ print $2, $1 }' filename.extension names.txt
```

This programs exchange first two fields by considering default field seperator in tab space.  
"," indicates that should followed by space, if you remove braces it will show errors.  
...
neither
```
Awk uses $ symbol for selecting fields.  

It display third field or column seperating default spaces as Field Seperator.  
"," acts as a output field seperator.  
If you remove "," in the first program it concatenates both the fields.  
...
Hughes Eve
```
Here __.awk__ extension is not necessary, but if you keep it is very informative.  
...
one,two,three
```
This only print second field which is seperated with space.  
If you enter "," field seperated date it doesn't understand and leaves it empty.  

To print "," seperated fields in the file you need to use -F which is field seperator.  

__Example:__

awk -F , '{ print $2 }' comma seperated fields  
awk -F t '{ print $2 }' tab seperated fields  
...
10  14:32:03 up  2:31,  2 users,  load average: 0.94, 0.58, 0.76
```
```awk
awk '{ print NF,$0 }' > awk.out
```
Output of awk can be redirected from terminal output to file.
```shell
...
^C
```
Above awk program runs sequentially in a program first it is set to default field seperator and then  
, is set to field seperator than $2 field was displayed because ; is used at FS=","  
...
```
$ cat onebigline.txt 
one,two,three!four,five,six!seven,eight,nine!ten,eleven,twelve[bhemanth@localhost Exercise Files]$ 
```
__Record seperator__  
```awk
...
```awk
$ awk 'BEGIN{RS="";FS="\n"} {name=$1;addres=$2;citystatezip=$3; print name","address","citystatezip}' multiaddress.txt 
...
Fletcher Flosi	fletcher.flosi@yahoo.com	221.394141603
```
Above one is wrong awk code, use below correct code for the second solution.  
```awk
$awk 'BEGIN{FS=",";OFS="\t"}{ print }' nameemailavg.csv 
Art Venere,art@venere.org,256.62394383
Lenna Paprocki,lpaprocki@hotmail.com,259.688783099
Donette Foller,donette.foller@cox.net,282.32979844
Simona Morasca,simona@morasca.com,139.51911647
...
16 names.txt 8 Carlos Guthrie
17 names.txt 9 Sophia Buck
18 names.txt 10 Vincent Mitchell
19 names.txt 11 Buffy Harris
20 names.txt 12 Reuben Miles
21 names.txt 13 Brendan Fowler
22 names.txt 14 Mason Hancock
23 names.txt 15 Nigel Boone
24 names.txt 16 Gretchen Foreman
25 names.txt 17 Serena Goodman
...
nor
```
This prints -1 because  $NF=$0, $NF-1=$0-1=-1  
...
^C
```
This points to value of first field.   
...
```
Awk doen't make any changes to original file, it will just print like above.  
...
```
In this it show 11 fieds even though are nothing and invisible.  
...
11!They!were!neither!up!nor!down!!!!!ELEVEN
```
Here it will show output field seperator with !.  
...
In this NF show 3 fields and $2=two in this, $0 value changes are effected.  
...
one two
 two
```
Here output one is not printed because $1=hello, but at printside it is ehllo instead of hello.  
...
__Note:__ it will display output in next enter.  
```awk
awk '{a=1;b=3;print ab}'
```
This will perform concatenation.  
...
1
```
Here it conside b as 0, because b value has string.  
Output will be 1.  
__Note:__ it will display output in next enter.  
```awk
$ awk '{ print "one" }'
...
0
```
It considers one as 0 because it is a string and add with other zero 0+0=0.  
...
```
```awk
$ awk '{ a=1; b=2; c=3; print ab*c}'
```
In this program awk multiplies & then contenates.  

```awk
$ awk '{a=1;b=2;c=3; print (ab)*c}'
```
Here first concatenation is forced then it multiplied.  
...
"-2.3"+0 = -2.3
66booboo
"66booboo"+0 = 66 -> Here booboo is ignore because number is at begin.  
booboo66
"booboo66"+0 = 0 -> Here even though number is present after a string it consider whole think as string and value become 0. 
```
```awk
...
- Increment and decrement operator: `++--`  
  - a=3  
  - b=++a; print a,b; # Pre-increment  
  - a=3
  - b=a++; print a,b; # Post-increment  
- Assignment operator: `=,+=,-=,*=,/=,%=,^=`  
- comparison operator: `==,!=,<,<=,>,>=`  
...
4 6 8 
4 6 8
$ awk '{a[1]=$1;a[2]=$2;a[3]=$3; print a[1],a[3],a[2]}'
1 2 3
1 3 2
```
...
- /abc/ matches "abc"
- /abc/ matches "xxabcxx"
- /abc/ doesnot match "abxxc"
- /abc/ doesnot match "ab"
- /abc/ doesnot match "ABC"
...
__^ and $ match string begin, end__  

- /^abc/ matches "abcd"  
- /^abc/ doen't match "dabc"  
- /abc$/ doesnot match "abcd"  
- /abc$/ matches "dabc"  
...
They were neither up nor down
```
It prints the or they or neither there in the third field, to restrict use ^ before that.
```awk
$ awk '$3~/^the/{ print}' dukeofyork.txt 
He marched them up to the top of the hill
And when they were up they were up
...
And when they were only half-way up
```
now only prints begin with "the"  

__`[]` matches any character in set__  

- /a[xyz]c/ matches "azc"  
- /a[xyz]c/ doesnot match "axyzc"  
- /a[a-z]c matches "abc"  
- /a[a-zA-Z]c/ matches "aBc"  
- /a[^a-z]c/ doesnot match "abc" -> This matches anything otherthan lowercase letter.  

__`*` matches zero or more repeats__  
...
__`+` matches one or more repeats__  

- `/ab+c/` matches `"abc"`  
- `/ab+c/` matches `"abbbbbbc"`  
- `/ab+c/` doesnot match `"ac"`  

__`?` matches zero or one repeats__  

- `/ab?c/` matches "abc"  
- `/ab?c/` doesnot match "abbbbc"  
- `/ab?c/` matches "ac"  

__`{ }` specifies a number of repeats__  

- `{n}` matches exactly n repeats  
- `/ab{}c/` matches "abbbc"  
- `/ab{}c/` doesnot match "abbbbc"  
- `{n,}` matches n or more repeats  
- `/ab{3,}c/` matches "abbbc"  
- `/ab{3,}c/` matches "abbbbbc"  

__`{n,m}` matches between n and m repeats__  

- `/ab{3,4}c/` matches "abbbc"  
- `/ab{3,4}c/` doesnot match "abbbbbc"  
- `/ab{3,4}c/` matches "abbbbc"  

__`()` repeats multiple items__  
...
```awk
$ awk '{ if(NF<8) { print "Shortline: ", $0}else{ print "longline: ", $0}}' dukeofyork.txt 
Shortline:  The grand old Duke of York
Shortline:  He had ten thousand men
longline:  He marched them up to the top of the hill
Shortline:  And he marched them down again
longline:  And when they were up they were up
longline:  And when they were down they were down
Shortline:  And when they were only half-way up
Shortline:  They were neither up nor down
```
...
```
awk -f scores_final.awk scores.txt 
<table>
<tr>
	<th>Bowler</th>
	<th>Round 1</th>
...
**Weekness of this code**  
  
  - Fails to ignore whitespace at beginning/end of line.  
  - Ignores lines where opening/closing tag differ in case.  
  - Ignore lines where tags has parameters(eg. `<a href>`).  
...
index("antidisestablishmentariansim","blish")  

- Returns 12 => because blish starts from 12th characters.  

*match() example* -> It looks for regular expression rather than string.  
...
```awk
$ awk -f removethe.awk dukeofyork.txt 
The grand old Duke of York
He had ten thousand men
...
r,A	art@venere.org	256.62394383
e,L	lpaprocki@hotmail.com	259.688783099
o,D	donette.foller@cox.net	282.32979844
i,S	simona@morasca.com	139.51911647
o,D	dominque.dickerson@dickerson.org	167.045197551
e,L	leota@hotmail.com	253.646335223
a,S	sage_wieser@cox.net	155.55576823
r,K	kris@gmail.com	210.794277775
i,M	minna_amigon@yahoo.com	95.47955397
b,A	amaclead@gmail.com	225.774477397
i,K	kiley.caldarera@aol.com	172.957628871
r,G	gruta@cox.net	202.68364784
o,J	josephine_darakjy@darakjy.org	178.877840188
a,C	calbares@gmail.com	290.446513401
a,M	mattie@aol.com	283.23995223
e,M	meaghan@hotmail.com	227.142916195
l,G	gladys.rim@rim.org	243.459635712
u,Y	yuki_whobrey@aol.com	128.321717297
l,F	fletcher.flosi@yahoo.com	221.394141603
```


```awk
$ awk -F, 'BEGIN{OFS="\t"}{ split($1,a,/ /); print a[2] "," a[1],$2,$3}' nameemailavg.csv 
Venere,Art	art@venere.org	256.62394383
Paprocki,Lenna	lpaprocki@hotmail.com	259.688783099
Foller,Donette	donette.foller@cox.net	282.32979844
Morasca,Simona	simona@morasca.com	139.51911647
Dickerson,Dominque	dominque.dickerson@dickerson.org	167.045197551
Dilliard,Leota	leota@hotmail.com	253.646335223
Wieser,Sage	sage_wieser@cox.net	155.55576823
Cho,Kris	kris@gmail.com	210.794277775
Amigon,Minna	minna_amigon@yahoo.com	95.47955397
Maclead,Abel	amaclead@gmail.com	225.774477397
Caldarera,Kiley	kiley.caldarera@aol.com	172.957628871
Ruta,Graciela	gruta@cox.net	202.68364784
Darakjy,Josephine	josephine_darakjy@darakjy.org	178.877840188
Albares,Cammy	calbares@gmail.com	290.446513401
Poquette,Mattie	mattie@aol.com	283.23995223
Garufi,Meaghan	meaghan@hotmail.com	227.142916195
Rim,Gladys	gladys.rim@rim.org	243.459635712
Whobrey,Yuki	yuki_whobrey@aol.com	128.321717297
...
__For-in statement:__ This set the variable index to defined each array  
...
```awk
$ awk -f wordusage.awk dukeofyork.txt | sort | more
again 1
and 4
down 4
duke 1
...
```awk
$ awk -f wordusage.awk dukeofyork.txt | sort -rn -k 2 | more
were 6
they 6
up 5
down 4
...
By using associative array we can use multi-dimensional array.  
...
```awk
$ awk -f transpose.awk dukeofyork.txt 
...
- int(x)  
- rand() -> random floating `int(rand()*6)+1` -> This returns random no. 1 to 6.  
- srand([x])  
- sqrt(x)  
- sin(x)  
- cos(x)  
...
```
$ ls -F 
addresses.txt*    excel.xls*           md.txt*            nicknames.txt*   scores_final.awk*  swap*
awk.out*          firstthree.awk*      multiaddress.txt*  officers.html*   scores.html        swap.awk*
csv.awk*          join.awk*            nameemailavg.csv*  officers.txt*    scores.txt*        transpose.awk*
dukeofyork.txt*   ls.out               nameemailavg.out*  onebigline.txt*  shortlong.awk*     wordusage.awk*
excel_final.awk*  makeofficers_final*  names.txt*         removethe.awk*   shortlong.sh*
```
```
$ ls -lF > ls.out
```
$ls -f : This shows executable file.  
...
Below command will not until you use sed command.  

```awk
$ ls -l | awk '/\.txt$/{ total += $5; print }END { print total }' ls.out
...
$ sed 's/*$//' ls.out
total 224
-rwxrwxrwx. 1 bhemanth bhemanth 80992 May 16  2014 addresses.txt
-rwxrwxrwx. 1 bhemanth bhemanth   281 Jun  2 14:34 awk.out
-rwxrwxrwx. 1 bhemanth bhemanth 10886 May 16  2014 csv.awk
-rwxrwxrwx. 1 bhemanth bhemanth   264 May 16  2014 dukeofyork.txt
-rwxrwxrwx. 1 bhemanth bhemanth   207 May 16  2014 excel_final.awk
-rwxrwxrwx. 1 bhemanth bhemanth 31744 May 16  2014 excel.xls
-rwxrwxrwx. 1 bhemanth bhemanth    79 May 16  2014 firstthree.awk
-rwxrwxrwx. 1 bhemanth bhemanth   370 May 16  2014 join.awk
-rw-rw-r--. 1 bhemanth bhemanth     0 Jun  8 20:08 ls.out
-rwxrwxrwx. 1 bhemanth bhemanth   316 May 16  2014 makeofficers_final
-rwxrwxrwx. 1 bhemanth bhemanth   659 May 16  2014 md.txt
-rwxrwxrwx. 1 bhemanth bhemanth   299 May 16  2014 multiaddress.txt
-rwxrwxrwx. 1 bhemanth bhemanth   921 May 16  2014 nameemailavg.csv
-rwxrwxrwx. 1 bhemanth bhemanth  1216 May 16  2014 nameemailavg.out
-rwxrwxrwx. 1 bhemanth bhemanth   262 May 16  2014 names.txt
-rwxrwxrwx. 1 bhemanth bhemanth  1309 May 16  2014 nicknames.txt
-rwxrwxrwx. 1 bhemanth bhemanth   555 May 16  2014 officers.html
-rwxrwxrwx. 1 bhemanth bhemanth   989 May 16  2014 officers.txt
-rwxrwxrwx. 1 bhemanth bhemanth    62 May 16  2014 onebigline.txt
-rwxrwxrwx. 1 bhemanth bhemanth   154 May 16  2014 removethe.awk
-rwxrwxrwx. 1 bhemanth bhemanth   518 May 16  2014 scores_final.awk
-rw-rw-r--. 1 bhemanth bhemanth  3017 Jun  5 12:40 scores.html
-rwxrwxrwx. 1 bhemanth bhemanth   794 May 16  2014 scores.txt
-rwxrwxrwx. 1 bhemanth bhemanth    88 May 16  2014 shortlong.awk
-rwxrwxrwx. 1 bhemanth bhemanth   109 May 16  2014 shortlong.sh
-rwxrwxrwx. 1 bhemanth bhemanth    15 May 16  2014 swap
-rwxrwxr-x. 1 bhemanth bhemanth    15 Jun  2 13:33 swap.awk
-rwxrwxrwx. 1 bhemanth bhemanth   241 May 16  2014 transpose.awk
-rwxrwxrwx. 1 bhemanth bhemanth   112 May 16  2014 wordusage.awk
```
...
```sed
$ sed 's/*$//' ls.out | awk '/\.txt$/{total+=$5;print}END{ print total}'
-rwxrwxrwx. 1 bhemanth bhemanth 80992 May 16  2014 addresses.txt
-rwxrwxrwx. 1 bhemanth bhemanth   264 May 16  2014 dukeofyork.txt
-rwxrwxrwx. 1 bhemanth bhemanth   659 May 16  2014 md.txt
-rwxrwxrwx. 1 bhemanth bhemanth   299 May 16  2014 multiaddress.txt
...
Johnna "Zip" Engelberg jlengelberg@engelberg.org
Buddy Cloney buddy.cloney@yahoo.com
Dalene Riden dalene.riden@aol.com
Jerry "Killer" Zurcher jzurcher@zurcher.org
...
```awk
$ awk '{if(NF==4){$2==""}; print}' nicknames.txt | awk '{ print $1,$2,"<"$3">"}' | more
Alex Loader <alex@loader.com>
Lashon Vizarro <lashon@aol.com>
Lauran Burnard <lburnard@burnard.com>
Ceola Setter <ceola.setter@setter.org>
...
^M is present at end of table line in microsoft office.  
...
```awk
$ awk -F t -f excel_final.awk excel.csv
!1,2,3
one,!wo,!hree
one one,!
```
...
```bash
$ cat officers.txt 
office	first_name	last_name	company_name	address	city	county	state	zip	phone1	phone2	email	web
President 	Annabelle	Boord	Corn Popper	523 Marquette Ave	Concord	Middlesex	MA	01742	978-697-6263	978-289-7717	annabelle.boord@cox.net	http://www.cornpopper.com
Vice President	Stephaine	Vinning	Birite Foodservice Distr	3717 Hamann Industrial Pky	San Francisco	San Francisco	CA	94104	415-767-6596	415-712-9530	stephaine@cox.net	http://www.biritefoodservicedistr.com
Secretary	Nelida	Sawchuk	Anchorage Museum Of Hist & Art	3 State Route 35 S	Paramus	Bergen	NJ	07652	201-971-1638	201-247-8925	nelida@gmail.com	http://www.anchoragemuseumofhistart.com
Treasurer	Marguerita	Hiatt	Haber, George D Md	82 N Highway 67Oakley	Contra Costa	CA	94561	925-634-7158	925-541-8521	marguerita.hiatt@gmail.com	http://www.habergeorgedmd.com
Member at Large	Carmela	Cookey	Royal Pontiac Olds Inc	9 Murfreesboro Rd	Chicago	Cook	IL	60623	773-494-4195	773-297-9391	ccookey@cookey.org	http://www.royalpontiacoldsinc.com
```

```bash
$ chmod +x makeofficers_final 
```

```bash
$ ./makeofficers_final officers.txt 
```
...
<ul>
<li>
Presiden  	Annabelle	Boord	Corn Popper	523 Marque : 
<a href="mailto:"></a>
</li>
<li>
Vice Presiden 	S ephaine	Vinning	Biri: 
<a href="mailto:r.com">r.com</a>
</li>
<li>
Secre ary	Nelida	Sawchuk	Anchorage Museum Of His  & Ar: 
<a href="mailto:"></a>
</li>
<li>
Treasurer	Margueri a	Hia : 
<a href="mailto:"></a>
</li>
<li>
Member a  Large	Carmela	Cookey	Royal Pon iac Olds Inc	9 Murfreesboro Rd	Chicago	Cook	IL	60623	773-494-4195	773-297-9391	ccookey@cookey.org	h: 
<a href="mailto:"></a>
</li>
</ul>
```
...
Andra	Scheyer	Ludcke, George O Esq	9 Church St	Salem	Marion	OR	97302	503-516-2189	503-950-3068	andra@gmail.comhttp://www.ludckegeorgeoesq.com
...
Salena	Karpel	Hammill Mfg Co	1 Garfield Ave #7	Canton	Stark	OH	44707	330-791-8557	330-618-2579	skarpel@cox.nethttp://www.hammillmfgco.com
...
Krissy	Rauser	Anderson, Mark A Esq	8728 S Broad St	Coram	Suffolk	NY	11727	631-443-4710	631-288-2866	krauser@cox.nethttp://www.andersonmarkaesq.com
...
Sylvie	Ryser	Millers Market & Deli	649 Tulane Ave	Tulsa	Tulsa	OK	74105	918-644-9555	918-565-1706	sylvie@aol.comhttp://www.millersmarketdeli.com
...
Gladys Rim	322 New Horizon Blvd, Milwaukee, WI 53207	243
Simona Morasca	3 Mcauley Dr, Ashland, OH 44805	139
Yuki Whobrey	1 State Route 27, Taylor, MI 48180	128
Dominque Dickerson	69 Marquette Ave, Hayward, CA 94545	167
Fletcher Flosi	394 Manchester Blvd, Rockford, IL 61109	221
Leota Dilliard	7 W Jackson Blvd, San Jose, CA 95111	253
Sage Wieser	5 Boston Ave #88, Sioux Falls, SD 57105	155
Kris Cho	228 Runamuck Pl #2808, Baltimore, MD 21224	210
Minna Amigon	2371 Jerrold Ave, Kulpsville, PA 19443	95
Abel Maclead	37275 St  Rt 17m M, Middle Island, NY 11953	225
Kiley Caldarera	25 E 75th St #69, Los Angeles, CA 90034	172
Graciela Ruta	98 Connecticut Ave Nw, Chagrin Falls, OH 44023	202
Josephine Darakjy	4 B Blue Ridge Blvd, Brighton, MI 48116	178
Cammy Albares	56 E Morehead St, Laredo, TX 78045	290
Art Venere	8 W Cerritos Ave #54, Bridgeport, NJ 08014	256
Mattie Poquette	73 State Road 434 E, Phoenix, AZ 85013	283
Lenna Paprocki	639 Main St, Anchorage, AK 99501	259
Meaghan Garufi	69734 E Carrillo St, Mc Minnville, TN 37110	227
...
[IBM](https://developer.ibm.com/tutorials/l-awk1/)  

[IBM Reference 2](https://developer.ibm.com/tutorials/l-awk2/)  

[IBM Reference 3](https://developer.ibm.com/tutorials/l-awk3/)  

[FAQ's](http://www.faqs.org/faqs/computer-lang/awk/faq/)  
